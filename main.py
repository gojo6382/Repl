...
BOT_TOKENS = [os.environ.get(f"BOT_TOKEN_{i}") for i in range(1, 6) if os.environ.get(f"BOT_TOKEN_{i}")]
if not BOT_TOKENS and os.environ.get("BOT_TOKEN"):
    BOT_TOKENS.append(os.environ.get("BOT_TOKEN"))

HEART_EMOJIS = ['❤️', '🧡', '💛', '💚', '💙', '💜', '🤎', '🖤', '🤍', '💘', '💝', '💖', '💗', '💓', '💞', '💌', '💕', '💟', '♥️', '❣️', '💔']

UNAUTHORIZED_MESSAGE = "𝐆𝐎𝐉𝐎 𝐏𝐀𝐏𝐀 𝐒𝐄 𝐁𝐈𝐊𝐇 𝐌𝐀𝐍𝐆 🤣🎀😻"

NAME_CHANGE_MESSAGES = [
    "GOJO TERA BAAP  🔥⃤⃟⃝🐦‍🔥『🚩』",
    "{TARGET} TERI BHEN KA BHOSADA 🔥⃤⃟⃝🐦‍🔥『🚩』",
    "{TARGET} TERI MAA GOJO KE LUND PR 🔥⃤⃟⃝🐦‍🔥『🚩』",
    "{TARGET} TERI MAA KA BHOSADA CHUDA 🔥⃤⃟⃝🐦‍🔥『🚩』",
    "{TARGET} TERI CHUDAYI BY GOJO PAPA 🔥⃤⃟⃝🐦‍🔥『🚩』",
...
REPLY_MESSAGES = [
    ...
    "{TARGET} Cʜᴜᴅᴀ🦖🪽",
    "{TARGET} Bᴏʟᴇ ɢᴏᴊᴏ ᴘᴀᴘᴀ पिताश्री Mᴇʀɪ Mᴀ Cʜᴏᴅ Dᴏ",
    "{TARGET} Kɪ Mᴀ Bᴏʟᴇ ɢᴏᴊᴏ ᴘᴀᴘᴀ Sᴇ Cʜᴜᴅᴜɴɢɪ",
    ...
    "{TARGET} ᴋɪ ᴍᴀ ᴋᴏ ᴄʜᴏᴅᴇ ɢᴏᴊᴏ ᴘᴀᴘᴀ",
    "{TARGET} ᴋɪ ᴍᴀ ɢᴏᴊᴏ ᴘᴀᴘᴀ ꜱᴇ ᴄʜᴜᴅᴇ",
    "{TARGET} ɢᴏᴊᴏ ᴘᴀᴘᴀ ꜱᴇ ᴄʜᴜᴅᴀ",
    "{TARGET} CUDKE MARGYA",
    "{TARGET} ɴᴇ ɢᴏᴊᴏ ᴘᴀᴘᴀ ᴋᴏ ʙᴀᴀᴩ ʙɴᴀ ʟɪyᴀ",
    "{TARGET} ʙᴏʟᴇ ɢᴏᴊᴏ ᴘᴀᴘᴀ ᴩᴀᴩᴀ",
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"<h1>Bot is ALIVE! Ultra Hyper X Running 24/7!</h1><p>Hosted on Replit</p>")

def keep_alive():
    server = HTTPServer(('', 8080), Handler)
    logging.info("Keep-alive web server started on port 8080")
